...
import { tools_node } from "../tools/index.js";
import { answer_node } from "../answer.js";
...
export function route(state: AgentState) {
  const [action] = state.actions.slice(-1);
  // 如果有相同的action，则不再继续
  if (
...
const builder = new StateGraph(StateAnnotation)
  ...
  .addConditionalEdges("agent_node", route, ["tools_node", "answer_node"])
