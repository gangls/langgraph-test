...
export const vectorTool = new DynamicTool({
...
export class VectorStoreTool extends StructuredTool {
  ...
  schema = z.object({
    query: z.string(),
    classification: z
      .enum(["Factual", "Analytical", "Contextual", "Opinion"])
      .describe("问题类型"),
  });
...
export const getTokens = async (query: string, topK = 10): Promise<Document[]> => {
  const keywords = tokenizeWithProtectedPhrases(query);
  if (keywords.length === 0){
    return [];
  }
  const filter = 'kg_category == "token" AND (' + keywords
    .map((keyword) => {
